...
function addToCart(drinkId: number, drinkData: DrinkData, drinkQty: number) {
	...
	if (currQty == undefined) {
		...
		cart.set(
			drinkId,
			new Pair<DrinkData, number>(drinkData, currQty + drinkQty)
		);
	}
}
...
export default function DrinkOrdering() {
	...
	const layout = useMemo(() => (width > 650 ? 'desktop' : 'mobile'), [width]);

	const { data, loading, error } = useGet<DrinkData[], DetailFormError>(
		'/drinks'
	);
	...
	return (
		<Content>
			...
			{!qtyPostive && (
				<p style={{ color: 'red' }}>
					The drink quantity must be a more than 0.
				</p>
			)}
			{drinkClicked && (
				<div>
